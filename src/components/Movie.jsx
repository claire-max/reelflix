...
const Movie = ({item}) => {
  ...
  return (
    ...
    <img className=' rounded-md w-full h-auto block' src={`https://image.tmdb.org/t/p/w500/${item?.backdrop_path}`} alt={item?.title}/>
...
<p onClick={saveShow}>
   {like ? (
     <FaHeart className='absolute top-4 left-4 text-pink-300 '/> 
     ):(
     <FaRegHeart className='absolute top-4 left-4 text-pink-300'/>
    )}
</p>
</div>
 </div>
